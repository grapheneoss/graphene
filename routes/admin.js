...
let settings = {
    panel:{
        ...
        name:(db.get("settings-panel") == undefined) ? "Graphene" : JSON.parse(db.get("settings-panel")).name
    },
    discord : {
...
router.get("/admin", async(req, res) => {
    if(req.session.loggedin !== true) {
        ...
        if(req.session.pterod.root_admin !== true) return res.redirect("/panel/home");
        req.session.servers = servers.data.data[0].attributes.relationships.servers.data;
        res.render("default/admin",{req:req,servers:req.session.servers,db:db,userinfo:req.session.userinfo,resources:availables,admin:req.session.pterod.root_admin,name:settings.panel.name,coins:JSON.parse(db.get(`coins-${req.session.userinfo.id}`)).coins})
    }
})
